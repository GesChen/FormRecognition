"""Normalize 10 images from cache, then run ID recognition (Tesseract) on the normalized images. Prints all results, including unidentified."""
...
ID_RECOGNITION["ocr_engine"] = "tesseract"
...
if __name__ == "__main__":
    paths = [CACHE_DIR / f"page_{i:04d}.png" for i in range(1, NUM_IMAGES + 1)]
    ...
    print("Step 1: Normalizing 10 images from cache (odd=a, even=b)...")
    page_keys = [template_key_for_page(int(Path(p).stem.split("_")[1])) for p in paths]
    normalized = normalize_images(paths, template_key=page_keys, cache_subdir="maury_1")
    print(f"  -> {len(normalized)} normalized")

    print("Step 2: ID recognition (Tesseract) on normalized images...")
    results = []
